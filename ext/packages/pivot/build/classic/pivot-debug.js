 ...
 * 
 * - only the following aggregator functions are used: min, max, avg, count, countNumbers, variance,
 * varianceP, stdDev and stdDevP
...
Ext.define('Ext.exporter.excel.PivotXlsx', {
    ...
    /**
     * If there are no dimensions on left/top axis then the pivot table needs to have a special
     * layout on the excel sheet. This function does that.
     *
     * @param params
     * @private
     */
    ...
    /**
     * The pivot table needs a list of all fields available in the source sheet and what unique
    ...
    generatePivotFields: function(params) {
        ...
        // for each pivot field, which fields are used on row/column axis and which fields
        // are data fields
        len = fields.length;
        for (i = 0; i < len; i++) {
            field = fields[i];
    ...
    /**
     * This function parses the matrix store and extracts all data and all unique values
     dimension = me.getDimension(matrix.aggregate, field)for each field.
     *
     * @param store
     * @return {Object}
     * @private
    ...
    getColItem: function(colIndex, aggIndex, columns) {
        var colItem = {
            ...
            len = columns.length,
            i, col;
        // we need to parse columns from top to bottom on the colIndex and find out what values
        // are empty so we can calculate r & x
        for (i = 0; i < len; i++) {
 ...
 *
 * For each summary function (ie `fn`) defined in this class there's a property name (ie `fnText`)
 * which will be used by the configurator plugin to display the function used for each aggregate
 * dimension.
 *
...
Ext.define('Ext.pivot.Aggregators', {
     ...
     *
     * **Note:** Used by the {@link Ext.pivot.plugin.Configurator configurator plugin} when listing
     * all functions that can be used on an aggregate dimension.
     */
    customText: 'Custom',
    /**
    ...
    sum: function(records, measure, matrix, rowGroupKey, colGroupKey) {
        ...
        // - if at least one value is not null then sum is not null
        //   (it's either 0 or some other number)
        for (i = 0; i < length; i++) {
...
Ext.define('Ext.pivot.filter.Base', {
     ...
     *
     * The `between` and `not between` operators expect this filter's {@link #value} to be an array
     * with two values.
     *
     * Possible values are:
     *
     ...
     * Value to filter by. For 'between' and 'not between' operators this should be an array
     * of values.
     */
    value: null,
    /**
    ...
    deprecated: {
        '6.0': {
            properties: {
                 ...
                 * @cfg {String} mztype
                 *
                 * @deprecated 6.0 Use {@link #type} instead. The old type config was renamed to
                 * {@link #operator}.
                 */
                mztype: null,
                /**
 ...
 *
 *  - Top/Bottom 10 Percent: Find first combined values to total at least x percent of the parent
 * grandtotal
 *
...
Ext.define('Ext.pivot.filter.Value', {
     ...
     * the results are sorted by the grand total in DESC (topOrder = top) or ASC (topOrder = bottom)
     * order.
     *
     */
    topSort: true,
...
Ext.define('Ext.pivot.axis.Item', {
    ...
    value: '',
    /**
     * @cfg {String/Number} sortValue The item sort value as it appears in the store. This value
     * will be used when sorting results.
     *
     */
    sortValue: '',
    /**
     ...
     * @readonly
     *
     * When the {@link Ext.pivot.matrix.Local Local} matrix is used this is the pivot store record
     * generated for this axis item. Only bottom level items belonging to the leftAxis have this
     * property.
     *
     */
    record: null,
    ...
    processRecord: function(record) {
        ...
        for (i = 0; i < dimCount; i++) {
            ...
            if (dimension.filter instanceof Ext.pivot.filter.Label) {
                // can't use the groupName to filter. That one could have html code in it
...
Ext.define('Ext.pivot.result.Base', {
     ...
     *
     * @param key The generated value will be stored in the result under this key for later
     * extraction
...
Ext.define('Ext.pivot.result.Local', {
     ...
     *
     * Array of records for the left/top axis keys pair. Only available for a
     * {@link Ext.pivot.matrix.Local Local} matrix.
     */
    records: null,
    constructor: function(config) {
...
Ext.define('Ext.pivot.matrix.Local', {
    ...
    onOriginalStoreUpdateDelayed: function() {
        var me = this,
            items = [],
            changed = false,
            len, i, records, record, obj, prev, current, sameLeft, sameTop;
 ...
 *
 * - keysSeparator: the separator used by the left/top items keys. It's the one defined
 *   on the matrix
 *
...
Ext.define('Ext.pivot.matrix.Remote', {
    ...
    processRemoteResults: function(options, success, response) {
        ...
        for (i = 0; i < len; i++) {
            item = items[i];
            if (Ext.isObject(item)) {
                result = me.results.add(item.leftKey || '', item.topKey || '');
 ...
 *
 *
 * When the exported data needs to be formatted then the
 * {@link Ext.pivot.dimension.Item#exportStyle} can be used on either left axis or aggregate
 * dimensions.
 *
 *      {
 *          xtype: 'pivotgrid',
...
Ext.define('Ext.pivot.plugin.Exporter', {
    ...
    prepareData: function(config) {
        var me = this,
            matrix, group, columns, headers, total, i, j, dLen, tLen, dataIndexes, row, value;
...
Ext.define('Ext.pivot.update.Base', {
    ...
    config: {
         ...
         * @cfg {String} dataIndex (required)
         *
         * Field that needs to be updated on all records found on the
         ...
         * @cfg {Variant} value
         *
         * The new value that is set for each record found on the
         * {@link Ext.pivot.result.Base matrix result}.
         */
        value: null
    },
 ...
 *      ]
 *
 * And we want to increment all orders by a fixed value of 50. This is how the updater config
 * looks like:
 *
 *      {
 *          type: 'increment',
...
/**
 * This updater overwrites the value on all records found on the
 * {@link Ext.pivot.result.Base matrix result}.
 *
 * Let's say that the result object contains the following records (each record is a
 * {@link Ext.data.Model model} in fact but we use json representation for this example):
...
/**
 * This updater evenly distributes the value across all records found on the
 * {@link Ext.pivot.result.Base matrix result}.
 *
 * Let's say that the result object contains the following records (each record is a
 * {@link Ext.data.Model model} in fact but we use json representation for this example):
 ...
 *      ]
 *
 * And we want to evenly distribute the value 300 to all orders. This is how the updater config
 * looks like:
 *
 *      {
 *          type: 'uniform',
...
Ext.define('Ext.pivot.plugin.configurator.Column', {
    ...
    afterRender: function() {
        ...
        // but the custom class we configure it on the component itself since it's more flexible
        // this way
...
Ext.define('Ext.pivot.plugin.configurator.DragZone', {
    ...
    getDDGroup: function() {
        // return the column header dd group so we can allow column droping inside the
        // grouping panel
...
Ext.define('Ext.pivot.plugin.configurator.DropZone', {
    ...
    getDDGroup: function() {
        // return the column header dd group so we can allow column dropping
        // inside the grouping panel
        return 'configurator-' + this.panel.id;
    },
    ...
    getLocation: function(data, t, e) {
        ...
        if (target instanceof Ext.pivot.plugin.configurator.Container) {
            // that means that the column is dragged above the grouping panel so find out
            // if there are any columns already
...
Ext.define('Ext.pivot.plugin.configurator.Panel', {
    ...
    placeholderExpand: function(animate) {
        ...
        if (animate) {
            ...
            // Unless we are expanding from a floated out situation. In that case, it's laid out
            // immediately.
...
Ext.define('Ext.pivot.plugin.Configurator', {
     ...
     * @param {Object} config
     * @param {Ext.pivot.plugin.configurator.Container} config.fromContainer Source container
     * to move from
     * @param {Ext.pivot.plugin.configurator.Container} config.toContainer Destination container
     * to move to
     ...
     * @param {Ext.pivot.plugin.configurator.Panel} panel Configurator panel
     * @param {Object} config
     * @param {Ext.pivot.plugin.configurator.window.Settings} config.container Window object where
     ...
     * @param {Ext.pivot.plugin.configurator.Panel} panel Configurator panel
     * @param {Object} config
     * @param {Ext.pivot.plugin.configurator.window.Settings} config.container Window object that
     * contains all pivot matrix settings.
     * @param {Object} config.settings Settings that will be loaded into the form
     */
    /**
     ...
     * @param {Object} config
     * @param {Ext.pivot.plugin.configurator.window.FieldSettings} config.container Window object
     * that contains all field settings.
     * @param {Object} config.settings Settings that will be loaded into the form
     */
    /**
     ...
     * @param {Ext.pivot.plugin.configurator.Field} config.field Field configuration
     * @param {String} config.container Type of container in which the field is located: `all`,
    ...
    config: {
         ...
         *
         * The fields are indexed by the dataIndex supplied to them which means that you can't have
         * two fields sharing the same dataIndex. If you want to define two fields that share
         * the same dataIndex then it's best to use a unique dataIndex for the 2nd field and define
         * a grouperFn on it.
         *
         * The dimensions that are configured on the pivot component but do not exist in this fields
         * collection
         * will be added here with a set of default settings.
         */
        fields: [],
        /**
         * @cfg {Number} refreshDelay Number of milliseconds to wait for pivot refreshing when
         * a config change occurred.
         */
        refreshDelay: 300,
        /**
         * @cfg {String} dock Docking position for the configurator panel. Possible values: top,
         * right, bottom, left
         */
        dock: 'right',
        /**
    ...
    onAfterPivotRendered: function() {
        ...
        // This is important because the dimensions may have labelRenderer/renderer/formatter
        // defined
...
Ext.define('Ext.pivot.plugin.RangeEditor', {
    ...
    onBeforeUpdate: function(updater) {
        var me = this,
            pivot = me.getGrid(),
            ret;
        ret = Ext.callback(me.onBeforeRecordsUpdate, me.scope || 'self.controller', [
            pivot,
            me.currentCol,
            me.currentRecords,
            updater.getValue(),
